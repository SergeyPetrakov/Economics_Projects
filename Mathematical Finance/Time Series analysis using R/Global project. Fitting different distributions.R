...
# sum returns
portf_eq         = portfolio[, .(date, pnl_eq     = return_1/3 + return_2/3 + retur_3/3)   ]
...
fit1 = fpot( t.ESM[,1],threshold=u[1], model="gpd", std.err = FALSE)
...
cdf2 = pgpd( t.ESM[,2],loc=u[2],scale=f?t2$par[1], shape=fit2$par[2] )
